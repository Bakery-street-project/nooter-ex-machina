...
export default {
  async fetch(request, env) {
    ...
    if (path === '/api/chat' && request.method === 'POST') {
      // Verify bearer token (or allow free-trial)
      const auth = request.headers.get('Authorization') || '';
      const rawToken = auth.replace('Bearer ', '');
      
      // Allow free-trial mode
      if (rawToken !== 'free-trial') {
      ...
      // NVIDIA NIM API - DeepSeek R1 (reasoning model)
      ...
      // Transform NVIDIA response to match expected format
      if (data.choices && data.choices[0]) {
        return json({
          content: [{ type: 'text', text: data.choices[0].message?.content || data.choices[0].text || '' }],
